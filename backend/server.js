...
setTimeout(() => {
  try {
    ...
    for (const profile of profiles) {
      ...
      if (profile.primary_type === 'individual') { keywords.add('individual'); keywords.add('personal'); }
      if (profile.primary_type === 'organization' || profile.primary_type === 'nonprofit') {
        keywords.add('nonprofit'); keywords.add('organization'); keywords.add('community');
      }
      ...
      if (sections.financial_information) { keywords.add('financial'); keywords.add('assistance'); }
      if (sections.narrative?.mission) {
        ['wellness', 'health', 'education', 'community', 'youth', 'senior', 'disability', 'veteran', 'faith', 'ministry', 'food', 'housing', 'employment']
          .forEach(term => { if (sections.narrative.mission.toLowerCase().includes(term)) keywords.add(term); });
      }
      ...
      const scored = opportunities.map(opp => {
        ...
        const oppTerms = new Set([...oppKeywords.map(k => k.toLowerCase()), ...oppCategories.map(c => c.toLowerCase())]);
        let keywordMatches = 0;
        keywords.forEach(kw => {
          for (const term of oppTerms) {
            if (term.includes(kw) || kw.includes(term)) { keywordMatches++; matchedFields.push(kw); break; }
          }
        });
      ...
      if (!orgId) {
        ...
        db.prepare(`INSERT INTO organizations (id, name, applicant_type, created_at, updated_at) VALUES (?, ?, 'individual_need', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`).run(orgId, profile.display_name || 'My Organization');
      ...
      for (const { opp, score, matchedFields } of topMatches) {
        const existing = db.prepare('SELECT id FROM grants WHERE organization_id = ? AND (funding_opportunity_id = ? OR title = ?)').get(orgId, opp.id, opp.title);
        if (!existing) {
          try {
            db.prepare(`
      ...
      if (added > 0) console.info(`[startup] Added ${added} grants for ${profile.display_name}`);
