...
import { filterByPolicy, enforceOpportunityPolicy } from '../services/crawlers/opportunityPolicy.js'
...
function buildCandidateOpportunityQuery({ crawlerType, profileContext, tokens, itemRequest, dialect }) {
  ...
  // Non-negotiable: exclude loans and matching-funds programs (always, not behind env).
  conditions.push(
    isPostgres
...
router.post('/run', ensureAuth, async (req, res) => {
  ...
  try {
    ...
    // Policy: drop loans, matching funds, placeholders, invalid URL (DB fallback path).
    const policyRejections = {}
    const { passed: scoredPolicyOk } = filterByPolicy(scored, { rejectionCounts: policyRejections })
