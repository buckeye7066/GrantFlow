...
router.get('/accounts', requireAdmin, async (req, res) => {
  ...
  const richQuery = req.db?.dialect === 'postgres'
    ? `SELECT ba.*, bt.name AS tier_name, bt.description AS tier_description, bt.base_monthly_cents AS tier_monthly, bt.hourly_rate_cents AS tier_hourly, bt.enable_pipeline_automation AS tier_enable_pipeline_automation, bt.enable_item_funding AS tier_enable_item_funding, bt.enable_document_ai AS tier_enable_document_ai, p.display_name AS profile_name, p.primary_type AS profile_type, p.status AS profile_status, o.applicant_type AS applicant_type, (SELECT ps.data FROM profile_sections ps WHERE ps.profile_id = p.id AND ps.section_key = 'basic_information' LIMIT 1) AS basic_section, (SELECT ps.data FROM profile_sections ps WHERE ps.profile_id = p.id AND ps.section_key = 'organization_details' LIMIT 1) AS org_section FROM billing_accounts ba LEFT JOIN billing_tiers bt ON bt.id = ba.tier_id LEFT JOIN profiles p ON p.id = ba.profile_id LEFT JOIN organizations o ON o.id = p.organization_id ORDER BY p.display_name ASC`
    : `SELECT ba.*, bt.name AS tier_name, bt.description AS tier_description, bt.base_monthly_cents AS tier_monthly, bt.hourly_rate_cents AS tier_hourly, bt.enable_pipeline_automation AS tier_enable_pipeline_automation, bt.enable_item_funding AS tier_enable_item_funding, bt.enable_document_ai AS tier_enable_document_ai, p.display_name AS profile_name, p.primary_type AS profile_type, p.status AS profile_status, o.applicant_type AS applicant_type, (SELECT ps.data FROM profile_sections ps WHERE ps.profile_id = p.id AND ps.section_key = 'basic_information' LIMIT 1) AS basic_section, (SELECT ps.data FROM profile_sections ps WHERE ps.profile_id = p.id AND ps.section_key = 'organization_details' LIMIT 1) AS org_section FROM billing_accounts ba LEFT JOIN billing_tiers bt ON bt.id = ba.tier_id LEFT JOIN profiles p ON p.id = ba.profile_id LEFT JOIN organizations o ON o.id = p.organization_id ORDER BY p.display_name COLLATE NOCASE ASC`
