...
--
-- Postgres mirror of migration 091_agent_control_center.sql. Tables:
--
--   agent_control_runs            top-level orchestration runs
--   agent_control_steps           one row per agent step in a run
--   agent_control_events          full audit trail (start/stop/heartbeat/etc.)
--   agent_control_locks           single-flight enforcement (full_cycle, per-agent)
--   agent_control_stop_requests   durable stop/pause/resume/cancel requests
--
-- Idempotent: every table/index uses IF NOT EXISTS.
...
CREATE TABLE IF NOT EXISTS agent_control_steps (
  ...
  cancellation_checked_at TIMESTAMPTZ,
