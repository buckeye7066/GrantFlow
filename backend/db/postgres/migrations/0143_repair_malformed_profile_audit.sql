-- Round 32: FORWARD migration (Postgres twin of sqlite 139_repair_malformed_profile_audit.sql).
-- Applies the round-31 BROADENED malformed-profile audit to DBs that already stamped an
-- INTERMEDIATE 0142 (the r30 LIKE-gated behavior), because the boot runner selects migrations
-- by FILENAME only, so the in-place r30/r31 edits to 0142 NEVER re-run on an already-0142-
...
-- IDEMPOTENT + safe after 0142: the sentinel canonical id makes ON CONFLICT DO NOTHING a
-- no-double-flag guard, so a row 0142 already flagged is untouched and a fresh install (0142
-- then 0143) is a safe no-op. Identical logic to sqlite 139 (pg_temp.pdedupe_is_json vs json_valid).
...
-- session-scoped, so this migration (a separate session from 0142) defines its own.
