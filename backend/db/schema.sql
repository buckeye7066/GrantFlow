...
CREATE TABLE IF NOT EXISTS funding_opportunities (
  ...
  entity_types_allowed TEXT DEFAULT '[]', -- JSON array; structured applicants
  need_types_supported TEXT DEFAULT '[]', -- JSON array; structured purposes
...
-- One-way history of rotated refresh-token hashes. This lets the auth route
-- distinguish an invalid random cookie from reuse of a formerly valid token,
-- while never retaining a bearer token in plaintext.
...
-- Human-controlled, external portal-submission receipt bindings. The evidence
-- bytes live in documents.file_bytes; this dedicated immutable binding (rather
-- than document type alone) is the authority used by submission proof checks.
CREATE TABLE IF NOT EXISTS hamilton_manual_submission_receipts (
  ...
  channel TEXT NOT NULL DEFAULT 'portal_manual' CHECK(channel = 'portal_manual'),
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_hamilton_manual_receipt_active_task
  ON hamilton_manual_submission_receipts(task_id) WHERE status = 'active';
...
FOR EACH ROW
WHEN EXISTS (
  SELECT 1 FROM hamilton_manual_submission_receipts WHERE document_id = OLD.id
)
BEGIN
...
-- (admin@grantflow.local) starts/stops/pauses/resumes the whole agent
...
-- Grant-scoped UI entities (migration 173 twin; fresh-install bootstrap).
-- Real persistence for ChecklistItem / GrantAward / ComplianceReport.
