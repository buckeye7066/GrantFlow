...
-- 063: Force re-application of admin diagnostics schema repair.
-- Some deployments had earlier repair migrations marked applied while the
-- physical grants/crawler_logs schema was still drifted. This migration keeps
-- the repair idempotent and traceable under a fresh migration id.
...
UPDATE grants
SET matched_needs = COALESCE(NULLIF(matched_needs, ''), COALESCE(NULLIF(match_reasons, ''), '[]'))
...
UPDATE grants
SET match_explanation = COALESCE(NULLIF(match_explanation, ''), json_object('source', 'admin_schema_repair', 'reason', 'Backfilled from linked funding opportunity'))
WHERE match_explanation IS NULL OR match_explanation = '';

UPDATE grants
SET fingerprint = COALESCE(NULLIF(fingerprint, ''), COALESCE(NULLIF(opportunity_fingerprint, ''), lower(hex(randomblob(16)))))
...
VALUES (
  ...
  '{"migration":"063_force_admin_schema_repair"}'
);

INSERT INTO crawler_logs (job_id, profile_id, crawler_type, level, status, message, payload)
SELECT id, profile_id, type, 'info', COALESCE(status, 'completed'), 'Backfilled profile-scoped crawler log for mission verification', '{"migration":"063_force_admin_schema_repair","source":"crawler_jobs"}'
FROM crawler_jobs
...
SELECT id, 'admin_schema_repair', 'info', 'completed', 'Backfilled profile-scoped crawler log for mission verification', '{"migration":"063_force_admin_schema_repair","source":"profiles"}'
FROM profiles
