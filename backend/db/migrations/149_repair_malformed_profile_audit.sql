...
-- that already stamped an INTERMEDIATE 148 (the r30 LIKE-gated behavior), because the boot
-- runner selects migrations by FILENAME only (files.filter(f => !applied.has(f))), so the
-- in-place r30/r31 edits to 148 NEVER re-run on an already-148-stamped DB — a NULL-phone,
...
-- malformed row). IDEMPOTENT + safe after 148: the sentinel canonical id makes
-- ON CONFLICT (dup_user_id, canonical_user_id) DO NOTHING a no-double-flag guard, so a row
-- 148 already flagged is untouched and a fresh install (148 then 149) is a safe no-op. The
-- Postgres twin (0153) uses identical logic (pg_temp.pdedupe_is_json instead of json_valid).
