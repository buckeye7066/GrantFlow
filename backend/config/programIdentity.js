 ...
 *
 * Declared BEFORE `stripProgramQualifiers` because a trailing " - …" clause is
 * only safe to strip when every word in it is one of these (or a year / stop
 * word). A blanket dash-strip collapsed distinct same-sponsor awards that
 * differ after the dash ("HEAP - Heating" vs "HEAP - Cooling") into one
 * identity — display hid the loser, and Robert's audit sticky-dismissed it.
 */
...
function isDashSuffixQualifierOnly(suffix) {
...
export function stripProgramQualifiers(title) {
  return String(title || '')
    ...
    .replace(/\bat\s+.*$/i, ' ')
    // Only strip " - Tennessee" / " - 2026" / " - Undergraduate" class suffixes.
    // Do NOT strip " - Heating Assistance" / " - Rent" — those name different
    // programs that happen to share a brand prefix and the same funder.
    .replace(/\s*[-–—]\s*([^]*)$/, (full, suffix) => (
      isDashSuffixQualifierOnly(suffix) ? ' ' : full
    ))
}
 ...
 *
 * The QUALIFIER_TOKENS set encodes: "Tennessee HOPE Scholarship" differs from
