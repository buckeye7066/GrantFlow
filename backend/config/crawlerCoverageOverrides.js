/**
 * crawlerCoverageOverrides.js
 *
 * Amy-managed, ADDITIVE-ONLY coverage overrides for the Crawler-OS source
 * registry. Amy's training loop detects categories that systematically return
 * zero results and broadens an existing, relevant source's coverage so those
 * profiles get candidates next run. Overrides can only ADD need_categories /
 * applicant_types to an existing source — they never remove or retarget, so a
 * bad override can only broaden recall (then be reverted), never silently break
 * an existing lane.
 *
 * Shape:
