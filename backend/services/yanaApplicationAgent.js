/**
 * yanaApplicationAgent.js
 *
 * Yana — application-completion agent. Given a profile + funding
 * opportunity (or an existing application_task), Yana:
 *
 ...
 *   5. Emits a persistent notification (yana_missing_info /
 *      yana_login_required / yana_application_ready / ...) so the user
 *      sees a toast + bell update.
 *   6. Records a `yana_runs` row so the agent telemetry / Mission
 *      Control surfaces Yana's activity.
 *
 * Yana NEVER:
...
import {
  ...
  getApplicationTask,
  TASK_STATUSES,
} from './yana/applicationTaskStore.js'
import { getFundingPortalLinkForOpportunity, linkOpportunityToPortal } from './yana/studentFundingPortalLinker.js'
import { getStudentPortal } from './yana/studentPortalStore.js'
import { emitYanaNotificationToProfileAndAdmins } from './yana/yanaNotifications.js'
...
export function isBrowserAutomationEnabled() {
  return String(ENV.YANA_ENABLE_BROWSER_AUTOMATION || 'false').toLowerCase() === 'true'
}

export function isAutoSubmitGloballyEnabled() {
  return String(ENV.YANA_ALLOW_AUTOSUBMIT || 'false').toLowerCase() === 'true'
}
...
function notificationTypeForOutcome(outcome, hasDocs, hasLogin) {
  switch (outcome) {
    case ADAPTER_OUTCOMES.READY: return 'yana_application_ready'
    case ADAPTER_OUTCOMES.SUBMITTED: return 'yana_application_submitted'
    case ADAPTER_OUTCOMES.DRAFT_COMPLETED: return 'yana_review_required'
    case ADAPTER_OUTCOMES.BLOCKED_LOGIN: return 'yana_login_required'
    case ADAPTER_OUTCOMES.BLOCKED_2FA:
    case ADAPTER_OUTCOMES.BLOCKED_CAPTCHA:
    case ADAPTER_OUTCOMES.BLOCKED_TERMS:
      return 'yana_application_blocked'
    case ADAPTER_OUTCOMES.BLOCKED_MISSING:
      return hasDocs ? 'yana_document_required' : 'yana_missing_info'
    case ADAPTER_OUTCOMES.WAITING_FOR_USER:
      return hasLogin ? 'yana_login_required' : 'yana_review_required'
    case ADAPTER_OUTCOMES.WAITING_FOR_ADMIN: return 'yana_review_required'
    case ADAPTER_OUTCOMES.FAILED: return 'yana_application_failed'
    default: return 'yana_review_required'
  }
}
...
async function recordYanaRun(db, summary) {
  try {
    ...
    // yana_runs is advisory — don't fail the agent run if logging fails.
    return null
  }
}
...
/**
 * Public entry point — start a new Yana run for a (profile, opportunity).
 * Idempotent: if a task already exists it is reused; this lets the
 * frontend "Let Yana continue" button resume mid-flight.
 */
export async function startYanaForOpportunity(db, {
  profileId,
  userId = null,
  opportunityId = null,
  grantId = null,
 ...
 * missing info and wants Yana to advance.
 */
export async function continueYanaTask(db, { taskId, actorUserId = null, actorRole = null } = {}) {
 ...
 *   yana_run_id,           // yana_runs id
 * }
 */
export async function runYanaCycle(db, {
  taskId,
  profileId,
  userId = null,
  actorRole = 'agent',
  ...
  if (notificationType) {
    notificationIds = await emitYanaNotificationToProfileAndAdmins(db, {
      profileId: task.profile_id,
...
function titleForNotification(type, opportunity, profile) {
  ...
  switch (type) {
    case 'yana_application_ready': return `Yana drafted ${oppTitle}`
    case 'yana_application_submitted': return `Application submitted: ${oppTitle}`
    case 'yana_review_required': return `Review required for ${oppTitle}`
    case 'yana_login_required': return `Login required for ${oppTitle}`
    case 'yana_document_required': return `Document needed for ${oppTitle}`
    case 'yana_missing_info': return `Missing information for ${oppTitle}`
    case 'yana_application_blocked': return `Yana blocked on ${oppTitle}`
    case 'yana_application_failed': return `Yana failed on ${oppTitle}`
    default: return `Yana update for ${studentName}`
  }
}
...
function messageForNotification(type, opportunity, _profile) {
  ...
  switch (type) {
    case 'yana_application_ready': return `Yana finished a draft for ${oppTitle}. Review and approve to submit.`
    case 'yana_application_submitted': return `Yana submitted ${oppTitle}.`
    case 'yana_review_required': return `${oppTitle} needs human review.`
    case 'yana_login_required': return `Yana needs you to log in to the school portal for ${oppTitle}.`
    case 'yana_document_required': return `Yana needs an uploaded document for ${oppTitle}.`
    case 'yana_missing_info': return `Yana needs additional information for ${oppTitle}.`
    case 'yana_application_blocked': return `Yana hit a security check (CAPTCHA/2FA/terms) on ${oppTitle}.`
    case 'yana_application_failed': return `Yana could not complete ${oppTitle}.`
    default: return `Yana has an update on ${oppTitle}.`
  }
}
