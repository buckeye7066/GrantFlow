...
export function scoreExtraction(meta = {}) {
  ...
  const avg = meta.avg_chars_per_page == null ? null : Number(meta.avg_chars_per_page)
  ...
  if (charCount >= 20_000) score += 0.10
    ...
    const ocrConf = meta.ocr_confidence == null ? null : Number(meta.ocr_confidence)
    const strongOcr = ocrUsed && ocrConf != null && Number.isFinite(ocrConf) && clamp01(ocrConf) >= 0.8
  ...
  const ocrConf = meta.ocr_confidence == null ? null : Number(meta.ocr_confidence)
  if (ocrUsed && ocrConf != null && Number.isFinite(ocrConf)) {
