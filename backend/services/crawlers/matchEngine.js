...
function computeNegativePenalty(program, analysis) {
  ...
  // Only apply negative matching when the program *requires* the condition (hard gate)
...
function isExcluded(program) {
  ...
  // Exempt legitimate programs that mention "loan" in passing
...
export function scoreProgram(program, analysis) {
  ...
  // Detect profile intents once (caller can cache, but cheap enough here)
...
export function matchPrograms(allPrograms, analysis, options = {}) {
  ...
  let stats = { total: 0, dupId: 0, dupUrl: 0, dupName: 0, excluded: 0, informational: 0, belowMin: 0, nullScore: 0 };
  ...
  for (const program of allPrograms) {
    ...
    // Dedup by URL (catches Foundation Locator etc. appearing under different IDs)
    const normUrl = normalizeUrl(program.url);
    if (normUrl && seenUrls.has(normUrl)) { stats.dupUrl++; continue; }
    if (normUrl) seenUrls.add(normUrl);

    // Dedup by name (fuzzy — catches "Foundation Center" vs "Foundation Center Locator")
  ...
  console.log(`[MatchEngine] ${stats.total} candidates → ${results.length} matched | ` +
    ...
    `filtered(info:${stats.informational} excl:${stats.excluded}) ` +
