...
function extractUrlsWithContext(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const rawUrl of matches) {
      ...
      const titleFromUrl = u.pathname ? decodeURIComponent(u.pathname.split('/').filter(Boolean).pop() || u.hostname) : u.hostname
      const title = titleFromLine || titleFromUrl || `Grant link (${u.hostname})`
