...
export async function auditMatchQuality(db) {
  const profiles = db.prepare('SELECT id, display_name, primary_type, tags FROM profiles LIMIT 50').all()
  ...
  for (const p of profiles) {
    const grants = db.prepare(grantQuery).all(p.id)
...
export async function verifyMissionGoals(db) {
  ...
  try {
    ...
    if (!hasDecision) {
      ...
      const withDecision = db.prepare('SELECT COUNT(*) as c FROM grants WHERE match_decision IS NOT NULL').get()?.c ?? 0
      const total = db.prepare('SELECT COUNT(*) as c FROM grants WHERE profile_id IS NOT NULL').get()?.c ?? 0
  ...
  try {
    ...
    const sections = db.prepare('SELECT DISTINCT section_key FROM profile_sections').all()
  ...
  try {
    const types = db.prepare('SELECT DISTINCT primary_type FROM profiles WHERE primary_type IS NOT NULL').all()
  ...
  try {
    const grants = db.prepare('SELECT COUNT(*) as c FROM grants WHERE profile_id IS NOT NULL').get()?.c ?? 0
    const profiles = db.prepare('SELECT COUNT(*) as c FROM profiles').get()?.c ?? 0
  ...
  try {
    ...
    if (!hasExplain) {
      ...
      const grants = db.prepare('SELECT match_explanation, match_decision, profile_fingerprint FROM grants WHERE profile_id IS NOT NULL LIMIT 100').all()
  ...
  try {
    ...
    if (!hasFp) {
      ...
      const grants = db.prepare('SELECT profile_fingerprint, opportunity_fingerprint, matcher_version FROM grants WHERE profile_id IS NOT NULL LIMIT 100').all()
  ...
  try {
    const sections = db.prepare('SELECT COUNT(DISTINCT section_key) as c FROM profile_sections').get()?.c ?? 0
  ...
  try {
    const logs = db.prepare('SELECT COUNT(*) as c FROM crawler_logs WHERE profile_id IS NOT NULL').get()?.c ?? 0
  ...
  try {
    const grants = db.prepare('SELECT match_reasons FROM grants WHERE profile_id IS NOT NULL AND match_reasons IS NOT NULL LIMIT 50').all()
  ...
  try {
    const statuses = db.prepare(`SELECT DISTINCT status FROM grants WHERE status NOT IN ('discovered', 'archived')`).all()
  ...
  try {
    const sessions = db.prepare('SELECT COUNT(*) as c FROM anya_sessions').get()?.c ?? 0
    const memories = db.prepare('SELECT COUNT(*) as c FROM anya_brain_memory').get()?.c ?? 0
  ...
  try {
    const toolUsage = db.prepare('SELECT COUNT(DISTINCT tool_name) as c FROM anya_tool_usage').get()?.c ?? 0
