...
export async function authorizeAttestation(db, {
  ...
  if (existing) {
    ...
    return rowToAuth(await db.prepare('SELECT * FROM yana_attestation_authorizations WHERE id = ?').get(existing.id))
  }
  const id = crypto.randomUUID()
  await db.prepare(
    `INSERT INTO yana_attestation_authorizations
  ...
  return rowToAuth(await db.prepare('SELECT * FROM yana_attestation_authorizations WHERE id = ?').get(id))
}
...
export async function revokeAttestation(db, id, reason = null) {
  ...
  return rowToAuth(await db.prepare('SELECT * FROM yana_attestation_authorizations WHERE id = ?').get(String(id)))
}
...
export async function listActiveAttestations(db, profileId) {
  ...
  const rows = await db.prepare(
    `SELECT * FROM yana_attestation_authorizations
