 ...
 * the actual primary product behavior, but Yana Autopilot now ships
 * real automation modes alongside manual import:
 *
 *   - pilot_manual_import         — paste/upload export from portal (legacy)
 *   - browser_autopilot           — Yana drives the portal unattended
 ...
 *
 * The provider catalogue is editable: a row in `yana_portal_providers`
...
const SEED_PROVIDERS = Object.freeze([
  Object.freeze({
    ...
    tos_notes: 'Public scholarship portal. Yana uses standard form completion only.',
  ...
  Object.freeze({
    ...
    tos_notes: 'University SSO. Yana stops for 2FA unless an active session is reused.',
  ...
  Object.freeze({
    ...
    captcha_likely: false,
    two_factor_likely: true,
    tos_notes: 'Federal site. Yana never types an FSA ID password — relies on saved session or vault reference.',
    adapter_name: 'genericUniversityFinancialAidAdapter',
  }),
  Object.freeze({
    id: 'common_app',
    ...
    captcha_likely: true,
    two_factor_likely: false,
    tos_notes: 'CAPTCHA likely on first login. Yana stops for CAPTCHA.',
...
export async function listPortalProviders(db) {
  ...
  let rows = []
  try {
    rows = await db.prepare('SELECT * FROM yana_portal_providers').all()
  } catch { rows = [] }
...
export const YANA_INTEGRATION_MODES = Object.freeze([
  'pilot_manual_import',
  'browser_autopilot',
