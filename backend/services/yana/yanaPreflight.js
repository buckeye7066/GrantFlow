 ...
 * the launch screen, so Yana doesn't have to stop mid-flight.
 *
...
import { isAuthorizationActive, listActiveAuthorizations } from './yanaAuthorizationStore.js'
...
export async function preflightSingleSource(db, {
  ...
  for (const f of REQUIRED_IDENTITY_FIELDS) {
    if (!nonEmpty(pickFirst(profile, f.paths))) {
      blockers.push({
        ...
        detail: `Yana needs ${f.key.replace(/_/g, ' ')} on the profile before she can fill applications.`,
      })
    }
  }
  ...
  if (requiresFinancialFields(opportunity)) {
    if (!nonEmpty(pickFirst(profile, ['financial_information.household_income', 'household.income', 'household_income']))) {
      warnings.push({
        ...
        detail: 'Need-based aid usually requires income on the application. Yana will leave it blank if absent.',
      })
    }
  }
  ...
  if (looksLikeStudentFunding(opportunity)) {
    if (!hasDocOfType(docs, (d) => /transcript/i.test(`${d.name || ''} ${d.type || ''}`))) {
      warnings.push({
        ...
        detail: 'Most scholarships require a transcript. Yana will fill what she can but flag the upload.',
      })
    }
    if (!hasDocOfType(docs, (d) => /personal\s*statement|essay|narrative/i.test(`${d.name || ''} ${d.type || ''}`))) {
      ...
      warnings.push({
        ...
        detail: 'Yana can generate one from profile essays when generate_narratives is authorized.',
      })
    }
  }
  ...
  if (classification.automation_type === 'portal') {
    if (!nonEmpty(classification.resolved_url)) {
      blockers.push({
        ...
        detail: 'Add an application_url on the funding opportunity before Yana can drive the portal.',
      })
    }
  }
  if (classification.automation_type === 'mail' && !nonEmpty(classification.mailing_address)) {
    blockers.push({
      ...
      detail: 'Add a mailing_address on the funding opportunity. Yana refuses to invent one.',
    })
  }
  ...
  // Yana will refuse to actually submit unless submit_applications is
  ...
  if (source?.options?.allow_auto_submit && !authorization.submit_applications) {
    blockers.push({
      ...
      detail: 'You asked Yana to submit on completion but did not authorize submit_applications.',
    })
  }
