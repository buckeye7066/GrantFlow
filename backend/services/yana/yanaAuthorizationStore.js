/**
 * yanaAuthorizationStore.js
 *
 * Persistence layer for Yana Autopilot's standing authorizations.
 *
 * The user grants Yana authority once per (profile, selected funding
...
export const YANA_AUTHORIZATION_TYPES = Object.freeze([
  'complete_forms',
  'upload_documents',
  'generate_narratives',
  'save_drafts',
...
export const YANA_AUTHORIZATION_SCOPES = Object.freeze(['profile', 'task', 'funding_source'])
...
export async function ensureYanaAuthorizationSchema(db) {
...
export async function recordAuthorizations(db, {
  ...
  authorizationTypes = [],
  authorizationText,
  authorizationVersion = 'yana-autopilot-v1',
  options = {},
  metadata = {},
} = {}) {
  ...
  for (const t of authorizationTypes) {
    if (!YANA_AUTHORIZATION_TYPES.includes(t)) throw new Error(`invalid authorization_type: ${t}`)
  }
  if (!YANA_AUTHORIZATION_SCOPES.includes(scope)) throw new Error(`invalid scope: ${scope}`)
  await ensureYanaAuthorizationSchema(db)
  ...
  for (const target of targets) {
    for (const type of authorizationTypes) {
      const existing = await db.prepare(
        `SELECT id FROM yana_authorizations
      ...
      if (existing) {
        await db.prepare(
          `UPDATE yana_authorizations SET
      ...
      await db.prepare(
        `INSERT INTO yana_authorizations
...
export async function listActiveAuthorizations(db, { profileId, fundingSourceId = null, taskId = null } = {}) {
  ...
  const rows = await db.prepare(
    `SELECT * FROM yana_authorizations
...
export async function revokeAuthorization(db, { id, reason = null } = {}) {
  ...
  const nowFn = db?.dialect === 'postgres' ? 'now()' : 'CURRENT_TIMESTAMP'
  await db.prepare(
    `UPDATE yana_authorizations
        SET revoked_at = ${nowFn}, revoked_reason = ?, updated_at = ${nowFn}
      WHERE id = ?`,
  ).run(reason ?? null, String(id))
  const row = await db.prepare('SELECT * FROM yana_authorizations WHERE id = ?').get(String(id))
...
export async function createAutopilotRun(db, {
  ...
  const nowFn = db?.dialect === 'postgres' ? 'now()' : 'CURRENT_TIMESTAMP'
  await db.prepare(
    `INSERT INTO yana_autopilot_runs
  ...
  const row = await db.prepare('SELECT * FROM yana_autopilot_runs WHERE id = ?').get(id)
  return rowToRun(row)
}
...
export async function updateAutopilotRun(db, runId, patch = {}) {
  ...
  await db.prepare(`UPDATE yana_autopilot_runs SET ${sets.join(', ')} WHERE id = ?`).run(...params)
  return await getAutopilotRun(db, runId)
}

export async function getAutopilotRun(db, runId) {
  if (!db || !runId) return null
  const row = await db.prepare('SELECT * FROM yana_autopilot_runs WHERE id = ?').get(String(runId))
  return rowToRun(row)
}

export async function listAutopilotRuns(db, { profileId = null, taskId = null, limit = 100 } = {}) {
  await ensureYanaAuthorizationSchema(db)
  let sql = 'SELECT * FROM yana_autopilot_runs WHERE 1=1'
  const params = []
