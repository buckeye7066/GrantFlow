/**
 * yanaAutopilotEngine.js
 *
 * Yana Autopilot — user-authorized **unattended** Playwright completion.
 *
 ...
 *      gates. Any of these is a HARD BLOCKER — Yana saves progress and
 ...
 *         - Yana sees a Submit button AND `submit_applications` is
 *           authorized AND no blocker shows up → click Submit.
 *         - Yana sees a Submit button AND `submit_applications` is NOT
...
const NAV_TIMEOUT_MS = Number(process.env.YANA_AUTOPILOT_NAV_TIMEOUT_MS) || 25_000
const STEP_TIMEOUT_MS = Number(process.env.YANA_AUTOPILOT_STEP_TIMEOUT_MS) || 8_000
const MAX_PAGES = Number(process.env.YANA_AUTOPILOT_MAX_PAGES) || 12
...
// Attestation labels Yana CAN auto-check when use_standing_attestation
...
// Hard-blocker labels Yana NEVER auto-checks.
...
async function detectFields(page) {
  ...
  return await page.evaluate(() => {
    ...
    for (const el of all) {
      ...
      el.setAttribute('data-yana-fid', `f${idx}`)
      out.push({
        fid: `f${idx}`,
        tag, type,
  ...
  return await page.$$eval('button, input[type="button"], input[type="submit"], a[role="button"]', (els, { rxList }) => {
    ...
    for (const el of els) {
      ...
      for (const r of rxList) {
        ...
        if (re.test(text)) {
          el.setAttribute('data-yana-btn', `b${out.length}`)
...
async function detectAttestationGate(page, { authorizations }) {
  ...
  for (const it of items) {
    ...
    if (STANDING_ATTESTATION_PATTERNS.some((rx) => rx.test(text))) {
      ...
      // Authorized — Yana may tick it later in fill loop.
    }
  }
  return null
}
...
/**
 * Run Yana Autopilot against a target URL.
 *
 * @param {object} arg
 * @param {string} arg.url               application URL
 * @param {object} arg.profile           pre-loaded profile bundle
 * @param {object} arg.authorizations    boolean flags from yanaPreflight.readAuthorizations
...
export async function runAutopilot({
  ...
  const screenshotsRoot = screenshotsDir || path.join(os.tmpdir(), 'yana-autopilot-screens')
    ...
    while (pagesVisited < MAX_PAGES) {
      ...
      for (const f of fields) {
        ...
        if (!authorizations.complete_forms && rule.key !== 'email' && rule.key !== 'first_name' && rule.key !== 'last_name') {
          // Without complete_forms authorization Yana only fills basic
      ...
      return { status: 'blocked', blocker_kind: 'no_progress', blocker_detail: 'Yana could not find a Next/Submit button to continue', filled_fields: filled, pages_visited: pagesVisited, trace }
    }
