/**
 * yanaPortalPolicyRegistry.js
 *
 * Per-host policy registry: tells Yana whether automation is permitted
 ...
 *
 * Yana NEVER ignores an explicit terms-prohibit-automation rule. When
 ...
 * in the `yana_portal_policies` table and win over the seed.
 */

import crypto from 'node:crypto'
import { normalizeHost } from './yanaCredentialSessionService.js'
...
const SEED_POLICIES = Object.freeze([
  Object.freeze({
    ...
    notes: 'Yana never types an FSA ID or submits FAFSA on behalf of the student. Use saved authenticated session only when the user authorized session reuse, otherwise produce a manual checklist.',
  }),
  Object.freeze({
    ...
    notes: 'Common App ToS forbids third-party agent submission. Yana produces a printable preparation packet only.',
  }),
  Object.freeze({
    portal_host: 'tn.gov',
...
export async function getPolicyFor(db, portalHost) {
  ...
  if (db) {
    ...
    const row = await db.prepare('SELECT * FROM yana_portal_policies WHERE portal_host = ? LIMIT 1').get(host)
    if (row) return rowToPolicy(row)
    // Suffix match: "wwww.mtsu.edu" should match "mtsu.edu".
    const parts = host.split('.')
    for (let i = 1; i < parts.length - 1; i += 1) {
      const suffix = parts.slice(i).join('.')
      const r = await db.prepare('SELECT * FROM yana_portal_policies WHERE portal_host = ? LIMIT 1').get(suffix)
...
export async function upsertPolicy(db, {
  ...
  const existing = await db.prepare('SELECT id FROM yana_portal_policies WHERE portal_host = ? LIMIT 1').get(host)
  if (existing) {
    await db.prepare(
      `UPDATE yana_portal_policies SET
    ...
    await db.prepare(
      `INSERT INTO yana_portal_policies
...
export async function listPolicies(db) {
  ...
  await ensureSchema(db)
  const rows = await db.prepare('SELECT * FROM yana_portal_policies ORDER BY portal_host').all()
