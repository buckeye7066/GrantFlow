/**
 * yanaResolvedFieldStore.js
 *
 * "Resolve once, reuse forever." When Yana asks the user for a
...
export async function saveResolvedField(db, {
  ...
  if (fieldValue === null || fieldValue === undefined || String(fieldValue).trim() === '') {
    throw new Error('fieldValue required (Yana never stores empty values)')
  }
  ...
  const existing = await db.prepare(
    `SELECT id FROM yana_resolved_fields WHERE profile_id = ? AND field_key = ? LIMIT 1`,
  ).get(String(profileId), key)
  if (existing) {
    await db.prepare(
      `UPDATE yana_resolved_fields SET field_value = ?, confidence = ?, source = ?,
    ...
    return rowToField(await db.prepare('SELECT * FROM yana_resolved_fields WHERE id = ?').get(existing.id))
  }
  const id = crypto.randomUUID()
  await db.prepare(
    `INSERT INTO yana_resolved_fields
  ...
  return rowToField(await db.prepare('SELECT * FROM yana_resolved_fields WHERE id = ?').get(id))
}
...
export async function getResolvedField(db, { profileId, fieldKey } = {}) {
  ...
  const row = await db.prepare(
    `SELECT * FROM yana_resolved_fields WHERE profile_id = ? AND field_key = ? LIMIT 1`,
  ).get(String(profileId), key)
...
export async function getResolvedFieldsAsMap(db, profileId) {
  ...
  const rows = await db.prepare(
    `SELECT * FROM yana_resolved_fields WHERE profile_id = ?`,
...
export async function listResolvedFields(db, profileId) {
  ...
  await ensureSchema(db)
  const rows = await db.prepare(
    `SELECT * FROM yana_resolved_fields WHERE profile_id = ? ORDER BY field_key`,
  ).all(String(profileId))
