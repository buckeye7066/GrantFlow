/**
 * yanaPaymentAuthorizationService.js
 *
 ...
 *
 * Each charge writes a `yana_blocker_resolution` row (handled by the
 * resolver, not here) so every cent Yana spends is auditable.
 */
...
export async function authorizePayment(db, {
  ...
  await db.prepare(
    `INSERT INTO yana_payment_authorizations
  ...
  return rowToAuth(await db.prepare('SELECT * FROM yana_payment_authorizations WHERE id = ?').get(id))
}

/**
 * Determine whether Yana may charge `amount_cents` to `category` for
...
export async function canPayFor(db, {
  ...
  const rows = await db.prepare(
    `SELECT * FROM yana_payment_authorizations
...
export async function recordCharge(db, {
  ...
  const nowFn = db?.dialect === 'postgres' ? 'now()' : 'CURRENT_TIMESTAMP'
  await db.prepare(
    `UPDATE yana_payment_authorizations
  ...
  return rowToAuth(await db.prepare('SELECT * FROM yana_payment_authorizations WHERE id = ?').get(String(authorizationId)))
}
...
export async function revokePaymentAuthorization(db, id, reason = null) {
  ...
  return rowToAuth(await db.prepare('SELECT * FROM yana_payment_authorizations WHERE id = ?').get(String(id)))
}
