...
async function ensureLinkSchema(db) {
  ...
  await db.exec(`
    CREATE TABLE IF NOT EXISTS application_portal_links (
      ...
      can_yana_attempt ${boolType} NOT NULL DEFAULT ${defFalse},
...
export function classifyFundingPortal({
  ...
  // ── Determine login / admin / can-yana flags ──────────────────────
  ...
  const canYanaAttempt = (winner === 'external_application' || winner === 'admissions' || winner === 'scholarship')
    && !requiresUserLogin
    && confidence >= 0.4
  ...
  return {
    ...
    can_yana_attempt: canYanaAttempt,
    ...
    if (existing?.id) {
      await db
        ...
        .run(
          ...
          classification.can_yana_attempt ? 1 : 0,
      ...
      .prepare(
        `INSERT INTO application_portal_links
           (id, profile_id, user_id, opportunity_id, grant_id, portal_id, school_id,
            ...
            can_yana_attempt, requires_user_login, requires_admin_review,
...
function rowToLink(row) {
  ...
  return {
    ...
    can_yana_attempt: Boolean(row.can_yana_attempt),
