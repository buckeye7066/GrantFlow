/**
 * yanaApplicationPacketGenerator.js
 *
 ...
 *   - PDF   (when YANA_PACKET_PDF != 'false' AND Playwright chromium is
...
const PERSONA = 'Yana — MBA-level grant writer with 20 years of experience'
...
narratives.statement_of_need
  ...
  || (() => {
    if (!nonEmpty(narratives.personal_statement)) {
      ...
      return '[ Yana has nothing to ground a Statement of Need in. Add a "statement_of_need" or "financial_hardship" essay to the GrantFlow profile and Yana will fill this section. ]'
    }
...
nonEmpty(narratives.personal_statement)
  ...
  : (() => {
      ...
      return '[ Yana has no personal statement to ground a narrative in. Add a personal-statement essay to the GrantFlow profile under essays.primary or essays.personal_statement. Yana will not invent narrative content. ]'
    })(),
  })
...
- State of residence: ${basic.state || '[ not on file ]'}`,
  ...
  sections.push({
    ...
    body:
`The following items must accompany this application. Yana will mark each item ✓ if it is already in the GrantFlow Documents system; otherwise the item is flagged and added to the missing-info list.
...
function buildInstructionLines({ automationType, address, fax, email, url, due, subject }) {
  ...
  switch (automationType) {
    case 'mail':
      lines.push('Print the Yana-generated application packet (DOCX or PDF).')
      lines.push('Sign every page that requires a signature. Yana never auto-signs anything.')
      ...
      else lines.push('[ MISSING — funder mailing address. Yana refuses to invent one. ]')
    ...
    case 'email':
      ...
      break
    case 'portal':
      lines.push('This funder accepts portal applications. Yana will open the supervised browser session.')
      ...
      break
    case 'pdf_docx':
      lines.push('The funder publishes a downloadable form. Yana has populated it from the profile.')
    ...
    case 'auto_profile':
      ...
      break
    default:
      lines.push('Yana could not determine the submission channel. Review the opportunity manually.')
  }
  return lines
}
...
async function tryBuildPdfFromHtml(html) {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'test') {
      console.warn(`[yanaPacket] PDF render skipped: ${err?.message || err}`)
    }
    return null
  }
}
...
function getPacketStorageDir() {
  const base = process.env.YANA_PACKET_STORAGE_DIR
    || path.join(os.tmpdir(), 'grantflow-yana-packets')
 ...
 * YANA_PACKET_STORAGE_DIR, insert document rows, and return the
 * resulting metadata.
 */
export async function generateAndSavePacket(db, {
  ...
  const docxId = await insertDocumentRecord(db, {
    ...
    type: 'yana_generated_application',
    filePath: docxPath,
