...
export async function assessHamiltonFundingSource(db, { profileId, opportunity = null, grant = null } = {}) {
  ...
  // minting a task that can only die silently. This MUST run BEFORE the trust
  // gate: a URL-less pointer always trips trust's no_real_url (display=false),
  // so ordering trust first turns every research lead into a generic
  // funding_source_disallowed — the handoff branch becomes unreachable for
  // exactly the rows it exists for (the wired-but-unreachable class), and
  // callers that only special-case pointer_research_lead never see it.
