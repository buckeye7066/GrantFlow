...
const HELP_REGISTRY = [
  ...
  },
  {
    ...
    description: 'Search and discover funding opportunities from all connected sources.',
    purpose: 'GrantFlow uses a single crawler framework (crawlerFramework.js) with pluggable source adapters for: Grants.gov API, USASpending.gov API, SAM.gov (when API key configured), and state portals. Profile-driven crawling reads your profile\'s location, needs, and type to determine what to search for. Results appear here as soon as crawling completes.',
    whoCanUse: 'all',
