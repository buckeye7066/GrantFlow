...
export async function ensureBillingSchema(db) {
  ...
  // The entitlement resolver is invoked by request middleware, including in
  // development/test databases where the numbered migration runner may not
  // have run. Keep its durable authority available under the same scoped
  // billing self-heal contract as billing_tiers and billing_accounts.
  await ensureBillingAddonSchema(db, isPostgres)

  await seedBillingTiersIfMissing(db)
  await syncCanonicalOrgTierPricing(db)
}

async function ensureBillingAddonSchema(db, isPostgres) {
  await db.exec(isPostgres ? `
