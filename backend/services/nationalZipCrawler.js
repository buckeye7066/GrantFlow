...
async function findSourcesForZip(zipInfo, db) {
  ...
  try {
    ...
    if (sources.filter(s => s.listing_type === 'OPPORTUNITY').length < MIN_SOURCES_PER_ZIP) {
      try {
    ...
    const opportunityCount = sources.filter(s => s.listing_type === 'OPPORTUNITY').length;
    const programCount = sources.filter(s => s.listing_type === 'PROGRAM').length;
