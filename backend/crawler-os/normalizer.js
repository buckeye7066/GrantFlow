...
export function normalize(candidate, verdict, ctx = {}) {
  ...
  return makeOpportunity({
    ...
    open_date: candidate.open_date ?? null,
    deadline: candidate.deadline ?? null,
    is_rolling: candidate.is_rolling === true,
    recurrence: candidate.recurrence ?? (candidate.is_rolling === true ? 'rolling' : null),
    source_status: candidate.source_status ?? null,
    first_published_at: candidate.first_published_at ?? null,
    required_documents: candidate.required_documents ?? [],
    application_method: candidate.application_method ?? null,
