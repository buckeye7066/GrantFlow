/**
 * Demo Student / MTSU end-to-end smoke (live profile + live catalog).
 *
 ...
 * Demo Student profile fixture from designatedProfiles.js, and asserts that:
...
const demo_stem_student = DESIGNATED_PROFILES.find(
  (p) => p.id === 'profile-demo-tennessee-stem-student',
)

describe('Demo Student / MTSU end-to-end pipeline', () => {
  it('finds the Demo Student fixture in the designated roster', () => {
    expect(demo_stem_student).toBeDefined()
    expect(demo_stem_student.primary_type).toBe('high_school_student')
  })

  it('builds profile signals that include student-aid keywords', () => {
    const signals = buildProfileSignals({ profile: demo_stem_student, sections: demo_stem_student.sections })
  ...
  it('catalog yields ≥15 student-aid candidates for the merged signal set', () => {
    const signals = buildProfileSignals({ profile: demo_stem_student, sections: demo_stem_student.sections })
    const intent = interpretFundingIntentRules(DEMO_STUDENT_QUERY)
    ...
    // off-campus / cost-of-attendance student query for Demo Student.
  ...
  it('also benefits a generic Tennessee college student profile (global rule)', () => {
    ...
    // student-aid pool — proving the change is global, not Demo Student-only.
    const generic = {
