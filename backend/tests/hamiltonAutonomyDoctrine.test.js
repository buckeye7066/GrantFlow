...
describe('resolveContactFormVerifications', () => {
  ...
  it('isContactShapedSubmission mirrors the engine vocabulary and refuses anything application-shaped', () => {
    ...
    // a contact-surface signal, school/org must not auto-settle (engine honesty).
    ...
    // URL / confirmation text saying "application"/"scholarship" wins even for name+email.
    expect(isContactShapedSubmission(
  ...
  it('closes a contact-shaped quarantine as "no application was submitted" and leaves a real one alone', async () => {
    ...
    // Short real scholarship form: contact-looking keys + application URL — must stay quarantined.
