...
describe('getProfilePortals', () => {
  ...
  it('treats a login/apply URL path as a portal even without application_method', async () => {
    db.prepare('INSERT INTO grants (id, profile_id, title, application_url) VALUES (?, ?, ?, ?)')
      .run('g1', 'p1', 'AcademicWorks Award', 'https://fixture-foundation.org/apply/scholarship')
    const { portals, mailFaxSources } = await getProfilePortals(db, 'p1')
    const derived = derivedOnly(portals)
    expect(derived).toHaveLength(1)
    expect(derived[0].portalHost).toBe('fixture-foundation.org')
    expect(mailFaxSources).toHaveLength(0)
  })
...
it.each([false,true])('joined catalog targets respect preferred alias and refuse known bad targets (refused=%s)', async (refused) => {
  _resetProfilePortalIndexSchemaCache()
  const db = makeDb()
  try {
    db.exec('ALTER TABLE funding_opportunities ADD COLUMN source_url TEXT')
    db.prepare('INSERT INTO profiles (id,display_name) VALUES (?,?)').run('target-p','Target Profile')
    const good = 'https://fixture-foundation.org/apply'
    const bad = 'https://alpha.grantable.co/login'
    db.prepare('INSERT INTO funding_opportunities (id,title,apply_url,application_url) VALUES (?,?,?,?)')
      .run('target-o','Foundation scholarship',refused ? bad : good,refused ? good : bad)
    db.prepare('INSERT INTO grants (id,profile_id,title,funding_opportunity_id,application_method) VALUES (?,?,?,?,?)')
      .run('target-g','target-p','Foundation scholarship','target-o','portal')
    const out = await getProfilePortals(db,'target-p')
    const derived = derivedOnly(out.portals)
    expect(derived.some(p => p.portalHost.includes('grantable'))).toBe(false)
    expect(derived).toHaveLength(refused ? 0 : 1)
    if (!refused) expect(derived[0].portalHost).toBe('fixture-foundation.org')
    const { suggestPortalLogin } = await import('../services/hamilton/hamiltonPortalLoginSuggester.js')
    const suggestion = await suggestPortalLogin({ db,profileId:'target-p',opportunityId:'target-o',allowAi:false })
    expect(suggestion.loginUrl).toBe(refused ? '' : good)
