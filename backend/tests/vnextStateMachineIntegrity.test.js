...
function scopedFixture(state = VNEXT_STATES.DISCOVERED) {
  return {
    application: {
      id: 'app-1',
      state,
...
describe('VNext transition integrity', () => {
  ...
  it('claims a one-step transition with compare-and-swap on the current logical state', async () => {
    ...
    expect(sql).toContain("COALESCE(state, stage, 'DISCOVERED') = ?")
