...
describe('durable canonical discovery', () => {
  ...
  it('passes the live profile and cancellation signal to the canonical engine and retains source failures', async () => {
    ...
    const result = await processCrawlerOsDiscoveryJob({ db: 'fixture-db', job: { profile_id: 'profile-a' }, signal, deadlineMs: 180000 })
    ...
    expect(discover).toHaveBeenCalledExactlyOnceWith({ db: 'fixture-db', profileId: 'profile-a', signal, deadlineMs: 60000, trigger: 'dispatcher' })
