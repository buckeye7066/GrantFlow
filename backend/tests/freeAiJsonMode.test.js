...
describe('opt-in structured output for compatible free routes',()=>{
 it('defaults to loopback Ollama when local-only mode is not explicitly disabled',()=>{
  const routes=getConfiguredFreeAiRoutes({});
  expect(routes).toEqual([expect.objectContaining({id:'ollama',baseURL:'http://127.0.0.1:11434/v1',model:'llama3.2'})]);
 });
