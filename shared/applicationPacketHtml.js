/**
 * applicationPacketHtml — the SHARED, pure HTML builder for a NON-PORTAL funding
 * source's application PACKET. Both the browser printout (applicationPacketPrint)
 * AND the server-side "save packet to Documents" endpoint render from this ONE
 * builder so the printed and saved packets are byte-for-byte the same document.
 *
 * Pure + dependency-free on purpose: no DOM, no React, no Vite aliases. It only
 * takes plain data and returns an HTML string, so Node (the backend route) can
 * import it via a relative path and the browser can import it via the @ alias.
 *
 * A packet states who the application is for, the funding source + its page, the
 * funder's contact block, and clear "Mail to / Fax to / Email to" instructions
 * plus the steps to apply off-portal.
 */

/** http(s)-only URL guard (inlined so this module has zero imports). */
export function safePacketUrl(u) {
  if (!u) return null
  try {
...
export function primaryInstruction(method, contact) {
  const m = String(method || '').trim().toLowerCase()
...
/**
 * Build the complete printable packet HTML document for one non-portal source.
 *
 * @param {object} args
 ...
 * @param {boolean} [args.autoPrint] include the auto-print + print button (browser
 *   printout). The saved-to-Documents copy passes false so the stored file is a
 *   clean static document with no scripts.
 * @param {string} [args.generatedAt] override the "Generated …" stamp (so the
 *   server can stamp a deterministic timestamp).
 * @returns {string} a full <!doctype html> document
 */
export function buildApplicationPacketHtml({ profileName = '', source, autoPrint = true, generatedAt = null } = {}) {
  const src = source || {}
  ...
  const title = src.title || src.host || 'Funding source'
  const pageUrl = safePacketUrl(src.url)
  const method = src.applicationMethod || ''
...
<body>
  ...
  <div class="sub">Generated ${escapeHtml(stamp)}</div>
...
/** Friendly default file name for the saved packet document. */
export function packetDocumentName(source) {
  const title = source?.title || source?.host || 'Funder'
  return `${String(title).trim()} application packet`
}

export default buildApplicationPacketHtml
