...
## 5af. Round 32 (FINAL) — the r30/r31 broadening was applied by editing 138/0142 in place; ship it as a forward migration (Codex re-review of commit `b8fb1a7e`)

### R32-1 [HIGH] An already-stamped DB never re-runs the broadened malformed-audit (in-place edit + filename-only runner)
The boot runner selects migrations by **filename** (`files.filter(f => !applied.has(f))`), yet r30 and r31 changed the malformed-audit **data behavior by editing `138`/`0142` in place**. A DB that already stamped an *intermediate* `138`/`0142` (the r30 `LIKE`-gated behavior) will **never** re-run the broadened, no-`LIKE` predicate — so a NULL-phone/no-map user whose malformed `basic_information` lacked the literal word `phone` (phone under a `contact` key / numeric-only / uppercase `PHONE`) stays **silently unflagged** there. Prod runs `138`/`0142` fresh on merge (correct), but this violates the r24 discipline — *never edit an applied migration to change data behavior; add a forward one* — and the owner's local/staging may hold an intermediate stamp. **Fix — new forward migrations `139` (SQLite) + `0143` (Postgres twin)** that perform ONLY the broadened malformed-profile conflict insert (`primary_phone IS NULL` AND not-in-proven-map AND `basic_information` unparseable → `pre-map-malformed-profile, manual review`; no text heuristic, no `LIKE`; validity-guarded so the INSERT never aborts; SQLite `json_valid=0`, Postgres a self-contained `pg_temp.pdedupe_is_json()`). **Idempotent + safe after `138`/`0142`:** the sentinel canonical id (`(unknown-malformed-profile)`) makes `ON CONFLICT (dup_user_id, canonical_user_id) DO NOTHING` a no-double-flag guard, so a row already flagged is untouched and a fresh install (`138` then `139`) is a safe no-op. `138`/`0142`'s r31 predicate is unchanged for fresh installs. Both dialects share byte-identical audit logic (only `json_valid` vs `pdedupe_is_json` differs).

**Tests (`otpPhoneDedupMigration.test.js`, 28):** an already-138-stamped DB that *missed* three malformed rows (contact-key, numeric-only, uppercase `PHONE`) → running `139` **flags all three** (`pre-map-malformed-profile, manual review`), nothing moved, no split (**red** if `139` is neutered → they stay unflagged); re-running `139` is a **no-op** (no double-flag); a fresh install (`138` then `139`) does **not** double-flag; a valid phoneless profile is **not** flagged; and `139`/`0143` are asserted byte-identical in their audit logic. All r17–r31 re-assert.

**Confirmation:** already-stamped DBs get the broadened malformed-audit via forward `139`/`0143`; fresh installs are a safe no-op; the forward migration is idempotent (sentinel + `ON CONFLICT DO NOTHING`); both dialects identical.
