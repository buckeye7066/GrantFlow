...
- **[HIGH #1] The repair survives 137's phone-null.** 138 identified duplicates by primary_phone, which 137 nulls first → the repair was a silent no-op. 147/0151 now capture the dup→canonical identity into a durable `phone_dedupe_map` BEFORE the null; 148/0152 repair from it (+ live-capture fallback + re-apply the phone fix so a stamped DB is repaired end-to-end).
...
- All r19–r25 preserved. 148/0152 byte-identical (parity-tested); idempotent + fresh no-op. `anya_brain_memory` dropped from the move list (its `user_id` was a comment, not a column).
...
- **[HIGH #2] Pre-map (52caf99-137) stamped DBs are no longer silently skipped.** 148/0152 reconstruct a nulled dup from a durable trace (the dup profile's basic_information phone matching a canonical's phone_otp credential) and repair it; what can't be reconstructed is left operator-visible, never a silent empty no-op.
- All r19–r26 preserved. 148/0152 bodies identical except the one documented JSON-extraction dialect line (parity-tested); idempotent + fresh no-op. A generated `phoneDedupeClassification.json` is the single source of truth for the classification.
...
- All r19–r28 preserved; 148/0152 remain byte-identical in the correctness-critical merge/collapse/phone-fix regions (parity-tested) and derive move/revoke from the same classification.
...
- All r19–r29 preserved; `148`/`0152` remain byte-identical in the correctness-critical merge/collapse/phone-fix regions (parity-tested).
...
- All r19–r30 preserved; `148`/`0152` remain byte-identical in the correctness-critical merge/collapse/phone-fix regions (parity-tested).
...
- **[HIGH] Already-stamped DBs now get the r31 broadened malformed-audit via NEW forward migrations `149` (SQLite) + `0153` (Postgres).** The boot runner selects migrations by filename, but r30/r31 changed the malformed-audit DATA behavior by editing `148`/`0152` in place — so a DB that already stamped an intermediate `148`/`0152` never re-runs the broadened predicate and a malformed row lacking the literal `phone` stays silently unflagged (a violation of the r24 "never edit an applied migration to change data behavior — add a forward one" discipline; the owner's local/staging may hold an intermediate stamp). `149`/`0153` perform ONLY the broadened conflict insert (`primary_phone IS NULL` AND not-in-proven-map AND `basic_information` unparseable → `pre-map-malformed-profile, manual review`; no `LIKE`; validity-guarded so it never aborts). Idempotent + safe after `148`/`0152`: the sentinel canonical id makes `ON CONFLICT DO NOTHING` a no-double-flag guard, so a fresh install (`148` then `149`) is a safe no-op and a re-run changes nothing. Both dialects share byte-identical audit logic.
- `148`/`0152`'s r31 predicate is unchanged; all r19–r31 preserved.

Compat: behavior-preserving; strictly delivers the already-earned broadening to DBs stamped before it via the correct forward-migration mechanism. New tests in `otpPhoneDedupMigration.test.js` (already-138-stamped miss → `149` flags all three malformed shapes; idempotent re-run; fresh-install no double-flag; valid not over-flagged; `149`/`0153` byte-identical audit logic), verified red by neutering `149`.
