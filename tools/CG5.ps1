...
Architecture: Express+Node20 backend (Railway), React18+Vite frontend (Vercel), PostgreSQL database.
...
function New-CodeGuardPullRequest([string]$HeadBranch,[int]$FixCount){
  ...
  $open = Invoke-GH "pulls?head=$headQ$([char]38)state=open$([char]38)base=$($script:M.BaseBranch)"
  $openList = @($open)
...
function Write-GH([string]$P,[string]$C,[string]$S,[string]$Msg,[string]$PrevContent=''){
  ...
  if($r){
    # Verify committed content matches what we intended (round-trip check)
    $verify=Read-GH $P
    if($verify -and $verify.c -ne $C){
      Write-Log "POST-COMMIT MISMATCH ${P}: committed content differs from intended. Manual review needed." 'ERROR'
      $script:S.Errs++
    }
    Write-Log "Committed: $P - $Msg" 'FIX';Learn-Fix $P $Msg;return $true
  }
...
function Invoke-Claude([string]$Sys,[string]$Usr,[int]$Mx=4096){
  ...
  try{
    ...
    # Model-aware cost: Sonnet=$3/$15, Opus=$15/$75, Haiku=$0.25/$1.25 per M tokens
    $inRate=3e-6;$outRate=15e-6
    if($script:M.Model -match 'opus'){$inRate=15e-6;$outRate=75e-6}
    elseif($script:M.Model -match 'haiku'){$inRate=0.25e-6;$outRate=1.25e-6}
    $cost=($inTok*$inRate)+($outTok*$outRate)
...
"@
          if($fixResult -and $fixResult.fixed -and $fixResult.original_code -and $fixResult.fixed_code){
            ...
            $sr=Invoke-SafeReplace $content $orig ([string]$fixResult.fixed_code) $targetFile
            if($sr.Matched){
              $content=$sr.Content
  ...
  # Report BROKEN mission gaps (architectural issues — report only, no auto-fix)
  # Mission gaps are cross-cutting concerns that span multiple files and often require
  # schema changes, migration coordination, or multi-file refactors. Single-file patches
  # create inconsistencies. These are flagged for human review instead.
  if($crossAnalysis -and $crossAnalysis.mission_gaps){
    ...
    if($brokenGaps -and @($brokenGaps).Count -gt 0){
      Write-Log "$(@($brokenGaps).Count) BROKEN mission gap(s) found - these require manual multi-file fixes (not auto-patched)" 'WARN'
      [void]$rpt.Add("")
      [void]$rpt.Add("## BROKEN Mission Gaps (require manual fix)")
      [void]$rpt.Add("These are architectural issues spanning multiple files. Auto-patching single files would create inconsistencies.")
      [void]$rpt.Add("")
      foreach($gap in $brokenGaps){
        [void]$rpt.Add("### Goal $($gap.goal) - BROKEN")
        [void]$rpt.Add("Evidence: $($gap.evidence)")
        [void]$rpt.Add("Recommended fix: $($gap.fix)")
        [void]$rpt.Add("")
      }
    }
  }
...
function Start-MatchAudit{
  ...
  foreach($p in $profiles){
    ...
    # Collect URLs during first pass (avoids redundant re-fetch later)
    foreach($g in $grants){
      if($g.url -and $allGrantUrls.Count -lt 30){
        [void]$allGrantUrls.Add(@{title=$g.title;url=$g.url;profile=$pname})
      }
    }
...
"@
  ...
  # --- URL HEALTH SPOT-CHECK (uses URLs collected during profile loop above) ---
  Write-Log "Spot-checking $($allGrantUrls.Count) grant URLs for dead links..." 'INFO'
