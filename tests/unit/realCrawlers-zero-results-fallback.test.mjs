/**
 * Zero-results fallback guarantee.
 *
 * User rule: "If total_found > 0 and included === 0, you must:
 *   - Log why items were removed
 *   - Relax constraints
 *   - Re-score"
 * And: "Zero results is a failure state, not an acceptable outcome."
 *
 * This test seeds a profile + DB where every row would normally be filtered
 * out by strict relevance rules (cross-state + demographic mismatch) and
 * verifies that the API STILL returns opportunities via the last-resort
 * fallback, with the drop_counts / threshold_fallback_message surfaced.
 */
...
test('zero-results fallback: total_found>0 guarantees opportunities.length>0 (non-strict mode)', async () => {
  ...
  try {
    ...
    // If allMapped was > 0 (i.e., total_found > 0), we MUST return something
    // thanks to the STAGE-2 / last-resort fallback.
    if (total_found > 0) {
      assert.ok(
