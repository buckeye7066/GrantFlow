...
test('ZIP closure is bounded, resumable, serialized, and explicitly production-bound', () => {
  ...
  assert.match(workflow, /grantflow-production-zip-closure/)
  assert.match(workflow, /GF_GITHUB_OIDC_TOKEN/)
  assert.match(workflow, /\$ACTIONS_ID_TOKEN_REQUEST_TOKEN/)
  assert.match(workflow, /\$\{ACTIONS_ID_TOKEN_REQUEST_URL\}/)
  assert.doesNotMatch(workflow, /env\.ACTIONS_ID_TOKEN_REQUEST_/)
  assert.doesNotMatch(workflow, /secrets\.ANYA_ADMIN_TOKEN/)
