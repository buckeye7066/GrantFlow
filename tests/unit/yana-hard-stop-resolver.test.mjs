/**
 * Yana Hard-Stop Resolver tests.
 *
 * Covers:
 *   - yanaBlockerClassifier.classifyBlocker — every category
 *   - yanaCredentialSessionService — record/find/expire/revoke
 *   - yanaPaymentAuthorizationService — authorize / canPayFor / charge
 *   - yanaAttestationStore — authorize + isAttestationAllowed
 *   - yanaPortalPolicyRegistry — getPolicyFor + upsertPolicy + seed
 *   - yanaResolvedFieldStore — save + get
 *   - yanaBlockerStore — recordBlocker + recordResolution
 *   - yanaHardStopResolver — every blocker category dispatch path
 */
...
import { classifyBlocker, BLOCKER_CATEGORIES } from '../../backend/services/yana/yanaBlockerClassifier.js'
import {
  recordSession,
  findValidSession,
  markSessionExpired,
...
} from '../../backend/services/yana/yanaCredentialSessionService.js'
import {
  authorizePayment,
  canPayFor,
  recordCharge,
...
} from '../../backend/services/yana/yanaPaymentAuthorizationService.js'
import {
  authorizeAttestation,
  isAttestationAllowed,
  listActiveAttestations,
  _resetAttestationSchemaCache,
} from '../../backend/services/yana/yanaAttestationStore.js'
import {
  getPolicyFor,
  upsertPolicy,
  listPolicies,
  _resetPortalPolicySchemaCache,
} from '../../backend/services/yana/yanaPortalPolicyRegistry.js'
import {
  saveResolvedField,
  getResolvedField,
  _resetResolvedFieldSchemaCache,
} from '../../backend/services/yana/yanaResolvedFieldStore.js'
import {
  recordBlocker,
  recordResolution,
  listBlockersForTask,
...
describe('yanaBlockerClassifier', () => {
...
describe('yanaCredentialSessionService', () => {
...
describe('yanaPaymentAuthorizationService', () => {
  beforeEach(resetCaches)
  ...
  it('authorizes, decides, charges, and refuses raw card data', async () => {
    ...
    const auth = await authorizePayment(db, {
      ...
      authorizationText: 'I authorize Yana to pay application fees up to $50.',
...
describe('yanaAttestationStore', () => {
  ...
  it('authorizes routine categories and matches by pattern', async () => {
    ...
    await authorizeAttestation(db, {
      ...
      authorizationText: 'I authorize Yana to confirm routine truthfulness statements.',
    })
...
describe('yanaPortalPolicyRegistry', () => {
  beforeEach(resetCaches)
...
describe('yanaResolvedFieldStore', () => {
  beforeEach(resetCaches)
...
describe('yanaHardStopResolver', () => {
  ...
  it('missing document → generates packet for Yana-generatable kinds', async () => {
    const db = makeDb()
