...
test('isolated Stripe acceptance covers lifecycle, entitlement, checkout, and webhook boundaries', () => {
  ...
  assert.match(workflow, /billingEntitlementAuthority\.test\.js/)
  assert.match(workflow, /billingEntitlementDecision\.test\.js/)
  assert.match(workflow, /billingUniversalEntitlement\.test\.js/)
  assert.match(workflow, /entitlementsMiddleware\.test\.js/)
  assert.match(workflow, /services-catalog-and-stripe\.test\.mjs/)
  assert.match(workflow, /stripe-price-verifier\.test\.mjs/)
  assert.match(workflow, /tier-enforcement\.test\.mjs/)
