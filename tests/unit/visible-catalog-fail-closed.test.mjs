...
describe('visible catalog fail-closed migration', () => {
  it('hides stale/unverified rows and preserves freshly verified rows', async () => {
    ...
    expect(sqlite.prepare('SELECT is_hidden FROM funding_opportunities WHERE id = ?').get('fresh').is_hidden).toBe(0)
    expect(sqlite.prepare('SELECT is_hidden FROM funding_opportunities WHERE id = ?').get('missing').is_hidden).toBe(1)
    expect(sqlite.prepare('SELECT is_hidden FROM funding_opportunities WHERE id = ?').get('stale').is_hidden).toBe(1)
  })
})
