/**
 * Unit tests for backend/services/yanaApplicationAgent.js + companion
 * stores. Verifies:
 *   - Yana detects missing info instead of hallucinating.
 *   - Yana blocks on CAPTCHA / 2FA / login (institutional aid portals).
 *   - Yana creates a persistent notification for missing info.
 *   - Yana never auto-submits without explicit authorization.
...
import {
  ...
  continueYanaTask,
  runYanaCycle,
} from '../../backend/services/yanaApplicationAgent.js'
import {
  getApplicationTask,
  listMissingInfo,
  listTaskEvents,
...
describe('yanaApplicationAgent — institutional aid', () => {
  it('blocks with login_required on financial-aid portal (FAFSA)', async () => {
    resetCaches()
    const db = makeMemoryDb()
    const result = await startYanaForOpportunity(db, {
      profileId: 'p-mtsu',
      userId: 'u-1',
    ...
    const notifs = db.raw.prepare("SELECT * FROM notifications WHERE type LIKE 'yana_%'").all()
    assert.ok(notifs.length >= 1, 'persistent notification emitted for the user')
  })
})

describe('yanaApplicationAgent — scholarship / external', () => {
...
describe('yanaApplicationAgent — profile scoping', () => {
  it('runYanaCycle rejects when profileId does not match the task', async () => {
    resetCaches()
    const db = makeMemoryDb()
    const start = await startYanaForOpportunity(db, {
      profileId: 'p-mtsu',
      userId: 'u-1',
      opportunityId: 'opp-fafsa',
    })
    await assert.rejects(
      () => runYanaCycle(db, { taskId: start.task.id, profileId: 'p-other', userId: 'u-2' }),
      /profile mismatch/i,
    )
  })
})

describe('yanaApplicationAgent — task events audit', () => {
