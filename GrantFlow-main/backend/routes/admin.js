 ...
 * Admin-only: delete a profile (hard-delete when possible; soft-delete on FK constraint).
 */
router.delete('/profiles/:id', async (req, res, next) => {
  try {
    ...
    try {
      await req.db.prepare('DELETE FROM profiles WHERE id = ?').run(id)
      return res.json({ ok: true, deleted: true, id })
    } catch (error) {
      // Fall back to soft-delete when hard-delete is blocked by FK constraints.
      await req.db.prepare("UPDATE profiles SET status = 'deleted' WHERE id = ?").run(id)
      return res.json({ ok: true, deleted: false, soft_deleted: true, id, reason: error?.message || String(error) })
    }
  } catch (error) {
    console.error('[admin/profiles/:id] delete error:', error)
    return next(error)
  }
})
