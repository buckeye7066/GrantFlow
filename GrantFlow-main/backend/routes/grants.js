...
const ALLOWED_GRANT_COLUMNS = new Set([
  'organization_id', 'funding_opportunity_id', 'profile_id', 'title', 'funder', 'deadline',
...
router.post('/from-opportunity', async (req, res) => {
  try {
    ...
    const normalizeDateOnly = (value) => {
      ...
      // Fast path for ISO timestamps ("2026-01-20T...")
    ...
    if (!opportunity && opportunity_data) {
      opportunity = {
        ...
        deadline: normalizeDateOnly(opportunity_data.deadline || opportunity_data.deadlineAt),
    ...
    // Normalize DB-backed opportunity deadline (Postgres grants.deadline is DATE).
    if (opportunity?.deadline) {
      opportunity.deadline = normalizeDateOnly(opportunity.deadline)
    }
    ...
    await req.db.prepare(`
      INSERT INTO grants (
        id, organization_id, funding_opportunity_id, profile_id, title, funder, 
