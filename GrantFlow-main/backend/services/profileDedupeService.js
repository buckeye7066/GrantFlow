...
async function addProfileEmail(tx, { profileId, email, addedBy = 'profile_merge' }) {
  const normalized = normalizeEmail(email)
  if (!normalized) return false

  const dialect = tx?.dialect || 'sqlite'
  if (dialect === 'postgres') {
    await tx
      .prepare(
        `
      ...
      .run(crypto.randomUUID(), String(profileId), normalized, addedBy)
    return true
  }

  // SQLite: profile_emails.id has a default; use INSERT OR IGNORE for uniqueness.
  await tx
    .prepare(
      `
        INSERT OR IGNORE INTO profile_emails (profile_id, email, added_by)
        VALUES (?, ?, ?)
      `,
    )
    .run(String(profileId), normalized, addedBy)
  return true
}

async function mergeProfileEmails(tx, { winnerId, loserId, actorUserId = null }) {
  await ensureProfileEmailSchema(tx)

  // Move existing email mappings from loser -> winner.
  const rows = await tx
    .prepare('SELECT email FROM profile_emails WHERE profile_id = ?')
    .all(String(loserId))
    .catch(() => [])
  for (const row of rows || []) {
    await addProfileEmail(tx, { profileId: winnerId, email: row?.email, addedBy: actorUserId || 'profile_merge' })
  }
  if (rows && rows.length) {
    await tx.prepare('DELETE FROM profile_emails WHERE profile_id = ?').run(String(loserId)).catch(() => null)
  }
}

async function ensureOwnerEmailsOnWinner(tx, { winnerId, winnerUserId, loserUserIds = [], actorUserId = null }) {
  await ensureProfileEmailSchema(tx)

  const userIds = Array.from(new Set([winnerUserId, ...(loserUserIds || [])].filter(Boolean).map(String)))
  if (userIds.length === 0) return

  const placeholders = userIds.map(() => '?').join(', ')
  const rows = await tx
    .prepare(`SELECT id, primary_email FROM users WHERE id IN (${placeholders})`)
    .all(...userIds)
    .catch(() => [])
  for (const row of rows || []) {
    if (!row?.primary_email) continue
    await addProfileEmail(tx, {
      profileId: winnerId,
      email: row.primary_email,
      addedBy: actorUserId || 'profile_merge',
    })
  }
}
...
export async function mergeProfiles(db, {
  ...
  return await db.withTransaction(async (tx) => {
    ...
    // Preserve email-based access: merge profile_emails and ensure owner emails remain mapped.
    await ensureProfileEmailSchema(tx)
    ...
    for (const loserId of uniqueLoserIds) {
      ...
      if (loser?.user_id) loserUserIds.push(String(loser.user_id))
      // Move any email mappings from loser to winner so email-based access is preserved.
      await mergeProfileEmails(tx, { winnerId, loserId, actorUserId })
    ...
    // Ensure the merged winner retains owner email mappings (winner + losers) for email-based access.
    await ensureOwnerEmailsOnWinner(tx, { winnerId, winnerUserId: winner?.user_id, loserUserIds, actorUserId })
