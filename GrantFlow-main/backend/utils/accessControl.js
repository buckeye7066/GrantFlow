...
export async function getAccessibleProfileIds(db, user) {
  ...
  // Trust session-scoped profileId.
  //
  // In production, authenticated sessions (JWT/session cookies) are issued by the backend and may carry
  // a `profileId` claim (e.g. `payload.profile_id` or `user_sessions.profile_id`).
  //
  // Some legacy/Base44-imported profiles may not have `profiles.user_id` populated yet, and may also be
  // missing `profile_emails` rows. In that case, the session-scoped profileId is the only reliable link
  // between the user and their profile.
  //
  // If we refuse to honor the session-scoped profileId, the user can be locked out (403) of their own
  // profile even while fully authenticated.
  const tokenProfileId = getAuthProfileId(user)
  if (tokenProfileId) {
    ids.add(String(tokenProfileId))
  }
