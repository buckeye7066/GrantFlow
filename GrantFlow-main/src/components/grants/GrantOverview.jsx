...
export default function GrantOverview({ grant, organization, onUpdate }) {
    ...
    const { data: matchedProfile } = useQuery({
        queryKey: ['profile', grant?.profile_id],
        queryFn: async () => base44.entities.Profile.get(grant.profile_id),
        enabled: Boolean(grant?.profile_id),
        staleTime: 5 * 60 * 1000,
    });
    const matchedProfileName = matchedProfile?.display_name || matchedProfile?.name || null;
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex-1">
                                ...
                                <p className="text-sm opacity-80 max-w-2xl">
                                    This opportunity was scored based on alignment with {matchedProfileName || organization?.name || 'your profile'}'s 
