...
|---|---|---|---|---|---|
| 1 | Yana→John daily leads→Drafts (cap 50, FROM Annie@axiombiolabs.org, never auto-send) | Yes | Yes (`JOHN_ENABLED`/`JOHN_RUN_ON_SCHEDULE`, daily 05:30 ET) | `john_runs` scheduled rows 06-29/06-30/07-01; drafts carry `actual_from: Annie@axiombiolabs.org`, `alias_send_supported: true`; cap defaults 50/24h; Annie mailbox confirmed real via Graph (200 on `/users/Annie@…/messages`) | Nothing to fix (volume is lead-supply-limited, not cap-limited). Owner-side: SendAs grant + `User.Read.All` remain optional polish (see §10.1) |
...
- **Send-as-Annie at MANUAL send time**: Graph accepts `from: Annie@axiombiolabs.org` on draft
  creation today (drafts verified). For the manual send from dr.johnwhite's Drafts to go out as
  Annie, Exchange needs a Send-As grant: M365 admin center → Users → Annie → Mail → *Send as*
  → add `dr.johnwhite@axiombiolabs.org` (or PowerShell:
  `Add-RecipientPermission "Annie@axiombiolabs.org" -Trustee "dr.johnwhite@axiombiolabs.org" -AccessRights SendAs`).
