 ...
 */
export async function getLastVisitedPath(profileId = null) {
  try {
    const prefs = await apiFetch('/api/preferences')
    const path = profileId ? prefs?.custom_preferences?.lastVisitedByProfile?.[profileId] : prefs?.custom_preferences?.lastVisitedPath
 ...
 */
export async function setLastVisitedPath(path, profileId = null) {
  if (!path || typeof path !== 'string') return
  try {
    const existing = await apiFetch('/api/preferences')
    const custom = existing?.custom_preferences ?? {}
    const merged = profileId ? { ...custom, lastVisitedByProfile: { ...(custom.lastVisitedByProfile || {}), [profileId]: path } } : { ...custom, lastVisitedPath: path }
