...
export const HARD_STOP_TARGETS = Object.freeze({
  ...
  captcha_required: {
    ...
    fixedWhere: "Portal + Saved portal logins",
    instructions: [
      "Under full automation with a configured solver, Hamilton attempts the CAPTCHA itself; this manual path is the fallback when that is unavailable.",
      "Open the portal and solve the CAPTCHA yourself, then save the resulting session on the Saved portal logins card.",
      "Hamilton resumes from the saved session.",
    ],
    href: savedLoginsHref,
  },
  payment_required: {
    actionLabel: "Review flagged source",
    fixedWhere: "Pipeline → flagged application",
    instructions: [
      "This source is asking for a payment to apply.",
      "Legitimate grants and funding sources never charge an application fee, so Hamilton did NOT pay and did not submit.",
      "Review whether this source belongs in the pipeline — Hamilton pays for nothing and there is no payment envelope.",
    ],
    href: humanReviewHref,
  },
