...
export default function AdminExclusionRules() {
  ...
  return (
        <div className="space-y-4">
          ...
          {/* Add new rule form */}
              <div className="flex gap-2 items-end border p-3 rounded bg-gray-50">
                      <div className="flex-1">
                                <label className="block text-xs font-medium mb-1">Pattern (text or regex)</label>
                                <input
                                              type="text"
          ...
          {loading ? (
                  <div className="text-sm text-gray-500">Loading exclusion rules...</div>
                ) : rules.length === 0 ? (
                  <div className="border border-dashed rounded p-6 text-center text-gray-500">
                            <p className="font-medium">No exclusion rules configured yet.</p>
                    ...
                    {rules.map(rule => (
                                <div key={rule.rule_id} className="border p-3 rounded flex items-center justify-between">
                                              <div>
                                                              ...
                                                              <div className="text-xs text-gray-500">Action: <span className="font-medium">{rule.action}</span></div>
                                              </div>
                                              <button
