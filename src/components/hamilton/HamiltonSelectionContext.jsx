...
/**
 * HamiltonSelectionContext
 *
 * Tracks which pipeline / GrantCard items the user has selected for the
 * "Automate selected with Hamilton" bulk action. Selection is keyed by a
...
// in `useHamiltonSelection.js` (react-refresh wants components-only files).
// eslint-disable-next-line react-refresh/only-export-components
export const HamiltonSelectionContextRaw = createContext(NoOpContext)
const Ctx = HamiltonSelectionContextRaw

function keyFor({ grant_id = null, opportunity_id = null } = {}) {
  return `${grant_id || ''}::${opportunity_id || ''}`
}

export function HamiltonSelectionProvider({ enabled = true, children }) {
...
// The hook lives in `./useHamiltonSelection.js` to keep this file
// component-only for react-refresh.
// eslint-disable-next-line react-refresh/only-export-components
export { useHamiltonSelection } from './useHamiltonSelection.js'
