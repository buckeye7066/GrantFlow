/**
 * YanaTaskBadge — compact summary the pipeline GrantCard renders to show
 * the linked portal + Yana status + action buttons. Progressive
...
export default function YanaTaskBadge({ profileId, grant, onTaskUpdated, onOpenDrawer }) {
  ...
  async function ensureAndStart() {
    if (!profileId || !grant?.id) {
      showErrorToast(toast, 'Yana cannot start', 'A profile and grant are required.')
      return null
    }
    setLoading(true)
    try {
      const created = await yanaApi.createApplicationTask({
        profileId,
        grantId: grant.id,
        opportunityId: grant.funding_opportunity_id || grant.opportunity_id || null,
      })
      const taskRow = created?.task || created
      const result = await yanaApi.startYana(taskRow.id, { trigger: 'pipeline_card' })
      setTask(result?.task || taskRow)
      if (result?.adapter_result?.message) {
        showInfoToast(toast, 'Yana update', result.adapter_result.message)
      } else {
        showInfoToast(toast, 'Yana started', `Working on “${grant.title}”.`)
      }
      onTaskUpdated?.(result?.task || taskRow)
      return result?.task || taskRow
    } catch (err) {
      showErrorToast(toast, 'Yana could not start', err?.message || 'See logs for details.')
      return null
    } finally {
      setLoading(false)
    }
