/**
 * YanaPortalsPanel — student-portal records that Yana uses to route
...
export default function YanaPortalsPanel({ profileId }) {
    ...
    async function load() {
      ...
      try {
        const [portalsResp, linksResp] = await Promise.all([
          yanaApi.listStudentPortals(profileId).catch(() => ({ portals: [] })),
          yanaApi.listFundingPortalLinks(profileId).catch(() => ({ links: [] })),
        ])
        if (cancelled) return
        setPortals(Array.isArray(portalsResp?.portals) ? portalsResp.portals : [])
        setLinks(Array.isArray(linksResp?.links) ? linksResp.links : [])
      } catch (err) {
        if (!cancelled) showErrorToast(toast, 'Yana portals failed to load', err?.message || 'See logs.')
      } finally {
    ...
    return (
      <Card>
        <CardContent className="flex items-center gap-2 py-6 text-sm text-slate-500">
          <Loader2 className="w-4 h-4 animate-spin" /> Loading Yana portal data…
        </CardContent>
      </Card>
    )
  }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            Yana portal connections
          </CardTitle>
        </CardHeader>
        <CardContent className="text-sm text-slate-500">
          Yana hasn't routed any funding opportunities yet. As soon as a discovered
          opportunity is added to the pipeline, Yana will connect it to the right
          school portal here.
        </CardContent>
      </Card>
    )
        ...
        {portals.map((p) => (
          <div key={p.id} className="border rounded p-3 space-y-1 bg-white">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline" className="text-xs">{yanaApi.portalTypeLabel(p.portal_type)}</Badge>
            ...
            <ul className="space-y-1 text-xs">
              {links.map((l) => (
                <li key={l.id} className="flex flex-wrap items-center gap-2 border-l-2 border-purple-200 pl-2">
                  <Badge variant="outline" className="text-[10px]">
                    {yanaApi.portalTypeLabel(l.portal_type)}
                  </Badge>
