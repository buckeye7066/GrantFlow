...
| `profile_names` | Comma-separated exact active profile names or unambiguous first names; use only with owner approval. Exactly one of `profile_ids` or `profile_names` is required, and ambiguous names fail closed. |
