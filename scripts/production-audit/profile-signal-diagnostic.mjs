...
const OUT_PATH = 'profile-scoring-diagnostic.json'
...
const SAFE_SECTION_FIELDS = Object.freeze({
  ...
  education: [
    'highest_level', 'current_institution', 'current_college', 'intended_major',
    'field_of_study', 'gpa', 'act_score', 'sat_score', 'target_colleges', 'interests',
  ],
  university_applications: ['applications'],
  financial_information: [
    'financial_need_level', 'low_income', 'unemployed', 'displaced_worker',
    'household_size', 'funding_amount_needed',
  ],
  government_assistance: [
    'medicaid_enrolled', 'medicaid_waiver_program', 'ecf_choices_role',
    'medicare_recipient', 'ssi_recipient', 'ssdi_recipient', 'snap_recipient',
    'tanf_recipient', 'section8_housing',
  ],
  health_medical: [
    'conditions', 'chronic_illness', 'chronic_illness_type', 'disability_type',
    'support_needs', 'support_needs_level', 'mental_health_condition',
    'substance_recovery', 'neurodivergent', 'visual_impairment',
    'hearing_impairment', 'wheelchair_user', 'tbi_survivor', 'amputee',
  ],
  family_life: [
    'single_parent', 'foster_youth', 'orphan', 'adopted', 'foster_parent',
    'caregiver', 'widow_widower', 'grandparent_raising_grandchildren',
    'first_time_parent', 'homeless', 'domestic_violence_survivor',
    'trafficking_survivor', 'disaster_survivor', 'formerly_incarcerated',
    'has_children', 'number_of_children',
  ],
  occupation: [
    'healthcare_worker', 'healthcare_worker_type', 'ems_worker', 'educator',
    'firefighter', 'law_enforcement', 'public_servant', 'clergy', 'missionary',
    'nonprofit_employee', 'small_business_owner', 'farmer', 'job_title', 'industry',
  ],
  demographics: [
    'african_american', 'hispanic_latino', 'asian_american', 'native_american',
    'tribal_affiliation', 'lgbtq', 'immigrant_status', 'gender', 'first_generation',
  ],
  military_service: [
    'veteran', 'active_duty_military', 'national_guard', 'disabled_veteran',
    'military_spouse', 'military_dependent', 'gold_star_family',
  ],
  ...
  if (Array.isArray(value)) {
    return value.slice(0, 20).map((entry) => {
      if (entry && typeof entry === 'object') {
        return { name: entry.name ?? null, status: entry.status ?? null }
      }
      return entry
    })
  }
...
function selectSafeSection(sectionKey, rawData) {
  ...
  for (const field of fields) {
    ...
    if (value !== null && value !== undefined && value !== '') selected[field] = value
  }
...
function evidenceSummary(raw) {
  ...
  let otherSubstantiveCredit = 0

  const compactMatched = matched.slice(0, 50).map((point) => {
    ...
    else if (!['geo', 'applicant_type'].includes(kind)) otherSubstantiveCredit += credit
    if (point?.via === 'declared_program') declaredProgramCredit += credit
    return {
      kind,
      value: String(point?.value ?? '').slice(0, 100),
      credit,
      via: point?.via ?? null,
    }
  })
  ...
  return {
    total: Number(evidence.total ?? breakdown.data_point_total ?? 0) || 0,
    total_credit: Number(evidence.total_credit ?? breakdown.data_point_total_credit ?? breakdown.data_point_credit ?? 0) || 0,
    ...
    other_substantive_credit: Math.round(otherSubstantiveCredit * 10) / 10,
    by_kind: byKind,
    matched: compactMatched,
    matched_needs: Array.isArray(explain.matchedNeeds) ? explain.matchedNeeds : [],
    eligibility_factor: breakdown.eligibility_factor ?? null,
    geography_factor: breakdown.geography_factor ?? null,
  }
}
...
try {
  ...
  `)).rows[0]
  if (identity.current_user !== 'grantflow_auditor') {
    throw new Error(`unexpected database role: ${identity.current_user}`)
  }
  if (identity.rolsuper === true) throw new Error('audit role is unexpectedly superuser')
  ...
  const matches = (await client.query(
    `WITH ranked AS (
       SELECT m.profile_id, m.opportunity_id, m.match_score, m.match_decision,
              ...
              o.title, o.sponsor, o.opportunity_kind, o.opportunity_type,
              o.funding_type, o.categories, o.keywords,
              ROW_NUMBER() OVER (
     ...
     SELECT * FROM ranked
      WHERE rn <= 250
      ORDER BY profile_id, match_score DESC, title`,
    [PROFILE_IDS],
  )).rows
  ...
  for (const row of matches) {
    ...
    matchesByProfile.get(row.profile_id).push({
      ...
      evidence: evidenceSummary(row.match_explain_json),
    })
  }
  ...
  for (const pattern of forbidden) {
    if (pattern.test(serialized)) throw new Error(`diagnostic output failed safety scan: ${pattern}`)
  }

  fs.writeFileSync(OUT_PATH, serialized)
  console.log(`wrote ${OUT_PATH}; profiles=${report.profiles.length}; matches=${matches.length}`)
} finally {
  await client.end()
}
