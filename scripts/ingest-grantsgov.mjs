#!/usr/bin/env node
/**
 * Ingest Grants.gov Opportunities
 * Fetches and ingests funding opportunities from Grants.gov API
 */
...
console.log('=== Grants.gov Ingestion ===\n');
...
async function main() {
  ...
  try {
    console.log('\nFetching opportunities from Grants.gov...');
    
    // Fetch opportunities
    const { opportunities, metadata } = await fetchGrantsGov({
      limit: 100, // Fetch first 100 opportunities
      offset: 0,
    });
    
    console.log(`\nFetched ${opportunities.length} opportunities from Grants.gov`);
