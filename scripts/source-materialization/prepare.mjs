...
function replaceExact(file, before, after, label) {
  ...
  if (first < 0 || text.indexOf(before, first + before.length) >= 0) throw new Error(`${label}: source block missing or ambiguous`)
...
replaceExact('backend/utils/runtimeSecrets.js', "const DEFAULT_PRODUCTION_KEY_FILE = '/data/grantflow-runtime-secrets.key'", "const DEFAULT_PRODUCTION_KEY_FILE = '/data/uploads/.grantflow-runtime-secrets.key'", 'runtime secret default key path')
replaceExact('backend/utils/runtimeSecrets.js', `  const uploadsDir = String(env.UPLOADS_DIR || '').trim()
...
const readiness = 'scripts/source-materialization/apply-readiness-deployment.mjs'
