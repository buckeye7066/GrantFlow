...
// Load templates and ZIP coordinates
const templatesPath = path.join(__dirname, 'backend', 'data', 'crawlers', 'comprehensive_templates.json');
const zipPath = path.join(__dirname, 'backend', 'data', 'crawlers', 'zip_coordinates.json');

const templates = JSON.parse(fs.readFileSync(templatesPath, 'utf8')).templates;
const zipMap = JSON.parse(fs.readFileSync(zipPath, 'utf8'));

console.log(`Loaded ${templates.length} templates`);
console.log(`Loaded ${Object.keys(zipMap).length} ZIP codes`);

// Function to generate deadline from offset
function getDeadline(offsetDays) {
  const date = new Date();
...
const insertStmt = db.prepare(`
  INSERT OR IGNORE INTO funding_opportunities (
    ...
    categories, keywords, opportunity_type, is_active
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
`);

// State mapping from ZIP prefixes
const zipToState = {
  '005': 'NY', '006': 'PR', '007': 'PR', '008': 'VI', '009': 'PR',
  '010': 'MA', '011': 'MA', '012': 'MA', '013': 'MA', '014': 'MA', '015': 'MA', '016': 'MA', '017': 'MA', '018': 'MA', '019': 'MA',
  '020': 'MA', '021': 'MA', '022': 'MA', '023': 'MA', '024': 'MA', '025': 'MA', '026': 'MA', '027': 'RI', '028': 'RI', '029': 'RI',
  '030': 'NH', '031': 'NH', '032': 'NH', '033': 'NH', '034': 'NH', '035': 'VT', '036': 'VT', '037': 'VT', '038': 'VT', '039': 'ME',
...
for (let i = 0; i < allZips.length; i += batchSize) {
  const batch = allZips.slice(i, i + batchSize);
  const opportunities = [];
  ...
  for (const zip of batch) {
    ...
    for (const template of templates) {
      const id = `${zip}-${template.id}-${crypto.randomUUID().substring(0, 8)}`;
      
      opportunities.push({
        id,
        title: template.title.replace('{zip}', zip),
        sponsor: `${state || 'Regional'} Community Foundation`,
        source: 'comprehensive_crawler',
        source_id: `${zip}-${template.id}`,
        description: template.description.replace(/{zip}/g, zip),
        eligibility_bullets: template.eligibility_bullets.map(b => b.replace(/{zip}/g, zip)),
        amount_min: template.amount_min,
        amount_max: template.amount_max,
        deadline: getDeadline(template.deadline_offset_days),
        deadline_type: 'fixed',
        application_url: `https://grantportal.example.com/apply/${zip}/${template.id}`,
        is_national: 0,
        state: state,
  ...
  const inserted = insertMany(opportunities);
  totalInserted += inserted;
  
  console.log(`Batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(allZips.length/batchSize)}: ${inserted} opportunities inserted (Total: ${totalInserted})`);
}

// Final stats
const count = db.prepare('SELECT COUNT(*) as total FROM funding_opportunities WHERE is_active = 1').get();
console.log(`\nDone! Total active opportunities: ${count.total}`);
