...
**MATCHER_VERSION: 4.2.0** — `computeMatchDecision()` is the sole
...
Current version: **4.2.0** (defined as `MATCHER_VERSION` in
`backend/services/matchEngine.js`; re-exported by
`backend/services/matchDecisionEngine.js`)

**Changes from v4.0.0 → v4.2.0 relevant to callers:**
...
| Path | Status | Notes |
|---|---|---|
| `backend/crawler-os/matchEngine.js` | ✅ canonical facade (v4.2.0) | Adapts Crawler OS opportunity/thesis rows into `computeMatchDecision`; contains no standalone decision weights |
...
| `backend/services/itemCrawler.js` | ✅ canonical (v4.2.0) | Uses `computeMatchDecision(profile, opp)` with explicit camelCase→snake_case mapping into `upsertFundingOpportunity` |
...
| `scripts/seed-profile-grants.mjs` | ✅ canonical (v4.2.0) | Dev-only; guarded by NODE_ENV/DISABLE_SEEDING; heuristic pre-score via canonical `scoreOpportunity`; `computeMatchDecision` is sole acceptance authority |
