...
## Final review boundary checks

The owner SDK wrapper now preserves native read-only model-list/key verification, and explicit adversarial author/verifier clients are provider-specific: they never silently share a fallback model or bypass the owner no-metered policy. Connection errors, SDK timeouts and HTTP 408/425 now use one shared transient classifier for fallback. Stop during owner planning returns the actual stopped response, persists cancelled=true, refreshes the thread, and suppresses a misleading new-answer toast. Eight new provider/cancellation assertions and two delivery assertions failed before repair; the final related verification passed 1,082 tests in 84 suites. Changed-file lint and whitespace checks passed.

## SDK deadline and native-diagnostic contract

Final review reproduced four additional assertions: an SDK fallback restarted the configured client timeout, retried its originating failed provider, did not abort a hanging native request at the shared budget, and returned an inference stub where optional key verification required null. The adapter now bounds the native attempt and subsequent fallback under one configured/per-call deadline, disables opaque SDK retries on that path, and excludes the originating provider from the fallback ladder. Owner inference without API keys is an explicit factory mode; native optional diagnostics retain configured:false. Supported inference callers request that mode. All 98 focused SDK/provider/owner tests passed after repair; the independently verified owner-email binding is merged locally without altering global administrator permissions.

## Explicit queued-work scope

Two actual queue-boundary regressions reproduced the remaining owner cancellation: document ingestion through the real dispatcher and the admin background fleet runner both inherited the finished HTTP request. They now capture only the existing server-authorized owner identity before deferral. Queue bookkeeping runs outside request scope, retries retain the same captured identity, and actual handlers use their own existing job timeout and cancellation signal; completion revokes the detached scope. Both explicit autonomous launchers follow the same bounded rule. Ordinary/shared jobs never acquire an owner scope. Timeout, already-cancelled capture, external abort, atomic claiming and durable completion are regression-covered.

## Complete enqueue and keyless-inference boundary

A full caller inventory found seven redundant deferrals around the already-asynchronous dispatcher (crawler routes, profile follow-through and tool registry). They are removed so authority is captured before the response, while actual job execution stays deferred inside the dispatcher. A syntax-tree regression checks these boundaries and every supported inference factory, including document ingestion, so keyless owner subscription calls cannot be gated by an absent paid key. Native key diagnostics and unsupported embedding/provider-native operations remain separate. Detached work now races its deadline even if a phase ignores the signal. Final-profile cancellation is rethrown before a completion report or audit. Fourteen new caller/budget assertions failed before correction; all 52 focused controls pass afterward.
