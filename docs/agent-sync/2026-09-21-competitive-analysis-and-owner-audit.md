...
VERIFIED follow-up: The desktop and 390px mobile Account browser fixture rendered with no uncaught browser errors or horizontal overflow. Plan comparison cards use canonical catalog prices; logout reached `/login`. Both original and recovery checkouts were reconciled by proving their complete staged trees equal the pushed commit. Full CI caught a missing Account breadcrumb label; the label was added without modifying the parity test.
