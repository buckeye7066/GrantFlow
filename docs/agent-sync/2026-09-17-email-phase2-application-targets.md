...
## Final corrective review 5242728407: shared opportunity response decorator
- The focused review found one remaining upstream conflation in decorateOpportunity: trust metadata promoted a source-only URL into application_url before the geographic UI received it.
- Added real SQLite-backed HTTP tests for list, detail and geo/scored, covering both source-only and conflicting explicit aliases. Both cases failed before correction. The common decorator now sets application_url only from resolveApplicationUrl(row) and never promotes meta.actionable_url. This fixes the shared response boundary, not just the mocked renderer.
- All22 affected HTTP/lifecycle/profile-scoring/trust/geo-renderer tests pass with exit0. Original stored source/application fields remain unchanged. All recorded review findings now have specific corrections and regression evidence; the final correction still requires complete prepush and current-head CI before merge.
