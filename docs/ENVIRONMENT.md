...
- **Free/local AI failover (OpenAI-compatible)**
  ...
  - JSON requests explicitly instruct OpenAI to return a complete JSON object. If OpenAI reports `finish_reason: length`, the helper can retry once with twice the original output limit, capped at 8,192 tokens, when at least one second remains in the existing paid-provider deadline. Requests already at that cap, other malformed responses, and exhausted deadlines are not retried. Truncated output is never accepted, even if a prefix parses. Recovery can use additional tokens; successful recovery reports aggregate token counters for both requests and logs `OpenAI JSON truncation recovered`. Final parse failures include the SDK finish reason, without response text.
...
- **Billing automation (invoices + reminders + suspension)**
  ...
  - A deleted profile (`profiles.status = 'deleted'`, or no profiles row after a
    ...
    lifecycle restore path brings a profile back.
