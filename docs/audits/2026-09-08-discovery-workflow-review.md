...
| After a completed scan, suffixed job keys let subsequent clicks miss the active rerun, duplicating work or colliding within one millisecond. | Archive terminal discovery keys and reserve the stable key for active work through centralized job creation and its unique index. | Reproduced failing concurrent-click regression after completion, then verified one shared rerun. |
...
| Faster browser journeys exhausted the fixture's shared 600-request/minute loopback bucket, blocking later profile-access and login requests. | Give only the isolated UX server capacity for the combined traffic of all fixture accounts; retain production limits and rate-limit test coverage. | Playwright traces showed HTTP 429 on access-gate and login requests after earlier journeys passed. |
