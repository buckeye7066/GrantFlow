...
| --- | --- | --- |
| `/real-crawlers/discover-all` awaited the whole live source fleet inside a proxied web request. UI comments incorrectly described a background dispatcher. | HTTP 202 now returns the ID of a persisted `crawler_os_discovery` job. Existing dispatcher claims, concurrency limits, heartbeats and recovery apply. Concurrent clicks join the active job. | Authorized route tests; durable enqueue and real dispatcher regression tests. |
